...
export default function AISearchInterface() {
  ...
  const FETCH_ALL = `${API_BASE_URL}/phones`;
  const GEMINI_URL = `${API_BASE_URL}/search`;
  ...
  const extractMainSpecs = (phone: Phone) => {
    ...
    const storage = storageMatch ? storageMatch[1] : phone.Memory?.internal || 'N/A';
        ...
        <div className={`transition-all duration-700 ${
          ...
          <div className="relative transition-all duration-700">
            ...
            <div className="relative backdrop-blur-xl rounded-2xl transition-all duration-700 bg-gray-800/80 border border-purple-500/30 shadow-2xl">
              <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 flex-wrap sm:flex-nowrap">
                <Search className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 text-purple-400 hidden sm:block" />
                ...
                <input
                  ...
                  className="flex-1 min-w-0 bg-transparent text-sm sm:text-base outline-none text-white placeholder-gray-400 w-full"
                />

                <div className="flex items-center gap-2 flex-shrink-0">
                  <button 
                    onClick={handleShowAllPhones} 
                    className="flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-2 rounded-lg bg-purple-500/20 border border-purple-400/50 text-purple-200 hover:bg-purple-500/30 transition-colors text-xs sm:text-sm whitespace-nowrap"
                  >
              ...
              {isLoading && (
                <div className="mb-6 sm:mb-8">
                  <div className="flex items-start gap-2 sm:gap-4 justify-start">
                    <div className="flex-shrink-0 w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center">
                      <Sparkles className="w-3.5 h-3.5 sm:w-5 sm:h-5 text-white animate-spin" />
                    </div>
                    <div className="backdrop-blur-xl rounded-2xl p-3 sm:p-4 bg-gray-800/60 border border-purple-500/20">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-300 ml-2 text-sm sm:text-base">Thinking...</span>
                      </div>
                    </div>
                  </div>
                </div>
